...
v 1.01, August 13, 2019
    first version
v 1.02, September 25, 2019
    m2kintro() and version_test() updated for Scopy version 1.1.0
v 1.10, October 12, 2019
    Add version parameter passing to m2kintro() to more easily track version changes from main program
    Minor updates
...
def m2kintro(program_version):
    ...
    :param program_version: String
        The program release version and date
    :return: None
    """
    intro_str1 = "Today's date: " + strftime("%A, %B %d %Y", localtime())
    intro_str2 = "Plot CSV text file data generated by ADALM-2000 Active Learning Module"
    intro_str3 = "Program: M2KScopePlot: " + program_version
...
def combine_file(file_path, file_base_name, file_extent):
    ...
    Example on Windows PC:
        combine_file('.\Preferences', 'preferences', 'ini') > ".\\Preferences\\preferences.ini"

    Example on Linux or MacOS:
        combine_file('.\Preferences', 'preferences', 'ini') > "./Preferences/preferences.ini"
    """
    # handle indirect path manually
    if file_path[0:2] == ".\\" or file_path[0:2] == "./":
        if name == "nt":
...
def get_datafile(config):
    ...
    """
    dir_default = config.get('Common', 'Test Data Location')
    if dir_default[0:2] == ".\\" or dir_default[0:2] == "./":
        if name == "nt":
            # use back slashes for directory name on Windows platforms
            dir_default = ".\\" + dir_default[2:]
        else:
            # for Linux, Unix, OSX use forward slashes for directory name
            dir_default = "./" + dir_default[2:]
    ext_default = "csv"
...
def read_pandas_info(config, nr=7, head=None, er_bad_lines=False):
    while True:
        dat_file = get_datafile(config)
...
def generate_plot_title(info_dict, tick_time, tick_units):
    freq_units, freq_value = rescale_frequency(float(info_dict['rate'][1]))
    plt_title = info_dict['device'][1] + " " + info_dict['tool'][1] + \
                " [" + "Time: " + str(round_sig(tick_time)) + " " + tick_units + "/div, " + \
                "Samples: " + str(info_dict['samples'][1]) + \
                ", " + info_dict['rate'][0] + ": " + \
                str(rescale_data(float(info_dict['rate'][1]), 10**freq_value)) + " " + freq_units + "]"
...
def get_plot_file(data_file):
    ...
    while True:
        ...
        while i < len(fig_name_list):
            if i == default_int:
                ...
                default_str = " "
            print(i, default_str, '\t', fig_name_list[i], '\t', fig_note_list[i])
            i += 1
        if i_max_note:
            print(i_max_note)
        print("Select graph file option:")
        print("\tpress integer for file")
        print("\tpress <enter> to select * default file name:", fig_name_list[default_int])
    ...
    if fig_name:
        temp_str = "Plot image saved to file: " + fig_name
    else:
        temp_str = "Plot image not saved to file"
    print(temp_str)
...
def select_channels_to_plot(headings):
    ...
    while True:
        ...
        for j in range(len(ans)):
            if ans[j] == chr(32):
                # skip <space> characters without reporting error
                continue
            try:
...
def select_custom_script(config, headings2Plot, sample_rate):
    """
    User info:
        Select an optional custom script to plot
        Allow user to change and remember parameters for the selected custom script
        When a parameter is changed, preferences.ini file is updated
    Developer info:
        When creating new custom scripts, add logic to perferences.ini and to main program - no changes needed here
    :param config: configparser object
    :return: String, List
            Script: A custom script to plot: "None" by default, "FFT", "Butterworth Filter", or perhaps a new script
            param_lst: A list of custom script parameters gathered from preferences.ini
    """

    # Initialize return option
    script = "none"

    default_key = 'None'

    # Allow user to select one of the suggested custom plot entries from preferences.ins
    while True:
        print('\nList of custom scripts:')
        i = 0
        custscript_names_lst = []
        for key in config['CustomScript Names']:
            if key == default_key:
                default_str = "*"
            else:
                default_str = " "
            custscript_names_lst.append(key)
            print(i, default_str, '\t', key)
            i = i + 1
        print("Select custom script:")
        print("\tpress integer for desired script")
        print("\tpress <enter> to select default *")
        ans = input("\t?")
        if ans == "":
            script = default_key
            break
        elif ans.isdigit() and 0 <= int(ans) < i:
            script = custscript_names_lst[int(ans)]
            break
        else:
            print("\nHelp: print one integer followed by <enter> key")
            print("Try again...\n")
            continue
    print("Custom script selected: " + script)

    # Allow user to make changes to custom script parameters
    config_updated = False
    # Control of initial display
    first_param_found = True
    # Avoid race condition by automating suggested best values for y_heading and srate only once
    automate_option_once = False
    while True:
        i_data = 0
        param_name_lst = []
        param_value_lst = []
        # Loop through all parameters within a custom script from preferences.ins
        for param_name, param_value in config.items(script):
            # Ignore parameters that are not proceeded with "Data "
            if param_name[0:5] == "Data ":
                param_name_data = param_name[5:]
            else:
                continue
            if first_param_found:
                first_param_found = False
                print('\nList of ' + script + ' Parameters:')
            param_name_lst.append(param_name_data)

            # Automate singe value option for y_heading and srate
            if param_name_data == "y_heading" and not automate_option_once:
                if len(headings2Plot) == 2:
                    # With only one y_heading to plot, automatically set the heading into preferences.ini
                    if param_value != headings2Plot[1]:
                        param_value = headings2Plot[1]
                        config.set(script, param_name, param_value)
                        config_updated = True
            elif param_name_data == "srate" and not automate_option_once:
                if param_value != sample_rate:
                    param_value = sample_rate
                    config.set(script, param_name, param_value)
                    config_updated = True

            if param_name_data != "srate":
                param_value_lst.append(param_value)
                print(i_data, " ", '\t', param_name_data, ":   ", param_value)
                i_data = i_data + 1
            else:
                param_value_lst.append(param_value)
                print("   ", '\t', param_name_data, ":   ", param_value)
        # Example FFT param_name_lst =  ['x_heading', 'y_heading', 'n', 'peak_cnt', 'min_peak_height']
        # Example FFT param_value_lst = ['', 'CH2(V)', '1', '0', '0.0']
        automate_option_once = True
        if not param_name_lst:
            break
        print("Select parameter to change:")
        print("\tpress integer for desired parameter")
        print("\tpress <enter> to accept current parameter list")
        ans = input("\t ?")
        if ans == "":
            break
        elif ans.isdigit() and 0 <= int(ans) < i_data:
            selection = int(ans)
            prompt_str = "Prompt " + param_name_lst[selection]
            prompt_str = config.get(script, prompt_str)
            if param_name_lst[selection] == "y_heading":
                heading_test = prompt_str.split("heading ")
                if len(heading_test) == 2:
                    temp_str = heading_test[0] + "heading ("
                    for i in range(1, len(headings2Plot)):
                        temp_str += headings2Plot[i] + ", "
                    prompt_str = temp_str[:-2] + ") " + heading_test[1]
            elif param_name_lst[selection] == "srate":
                heading_test = prompt_str.split("rate ")
                if len(heading_test) == 2:
                    temp_str = heading_test[0] + "rate ("
                    prompt_str = temp_str + str(sample_rate) + ") "  + heading_test[1]
            ans = input("\t" + prompt_str + " " + param_name_lst[selection] + " ?")
            if ans == "":
                continue
            param_name_str = "Data " + param_name_lst[selection]
            config.set(script, param_name_str, ans)
            config_updated = True
            continue
        else:
            print("\nHelp: print one integer followed by <enter> key")
    ...
    if config_updated:
        with open(combine_file('.\Preferences', 'preferences', 'ini'), "w") as config_file:
            config.write(config_file)

    return script, param_value_lst
